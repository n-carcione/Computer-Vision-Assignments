...
def main():
    ...
    # Q1.3
    # img_path = join(opts.data_dir, 'kitchen/sun_aasmevtpkslccptd.jpg')
    # img_path = join(opts.data_dir, 'windmill/sun_agudhwulyxcizdjv.jpg')
    # img_path = join(opts.data_dir, 'park/labelme_evuftvyfpanmmab.jpg')
    # img = Image.open(img_path)
    # img = np.array(img)
    ...
    # Use for 3.1 (run and evaluate whole system)
    n_cpu = util.get_num_CPU()
    # visual_words.compute_dictionary(opts, n_worker=n_cpu)
    # visual_recog.build_recognition_system(opts, n_worker=n_cpu)
    conf, accuracy = visual_recog.evaluate_recognition_system(opts, n_worker=n_cpu)    
    ...
    print(conf)
    print(accuracy)
    np.savetxt(join(opts.out_dir, 'confmat_cust.csv'), conf, fmt='%d', delimiter=',')
    np.savetxt(join(opts.out_dir, 'accuracy_cust.txt'), [accuracy], fmt='%g')
